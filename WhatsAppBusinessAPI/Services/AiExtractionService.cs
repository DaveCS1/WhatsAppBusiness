...
- TourTime: What time they prefer";
        ...
        public async Task<string?> GenerateCustomResponseAsync(string messageText, string context = "")
        {
            if (string.IsNullOrEmpty(_aiApiKey))
            {
                _logger.LogWarning("AI API Key is missing. Cannot generate custom response.");
                return null;
            }

            try
            {
                var prompt = $@"You are a helpful customer service assistant for a tour company. 
Generate a friendly, professional response to this customer message.
...
- Keeps the response concise (under 200 words)";
                ...
                {
                    var errorContent = await response.Content.ReadAsStringAsync();
                    _logger.LogError("AI API request failed for custom response. Status: {StatusCode}. Error: {ErrorContent}", 
                        response.StatusCode, errorContent);
                    return null;
                }
            ...
            {
                _logger.LogError(ex, "Error generating custom AI response for message: {MessageText}. Error: {Message}", 
                    messageText, ex.Message);
                return null;
            }
        }
